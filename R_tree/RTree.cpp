...
Rect RTree::MBR(vector<pair<int, int>> pol)
{
	...
	int y1 = pol[0].second;
	int y2 = pol[0].second;
	if (pol.size() == 1) { x1 -= 5; x2 += 5; y1 -= 5; y2 += 5; }
	else {
		for (unsigned int i = 1; i < pol.size(); i++) {
			if (pol[i].first < x1) x1 = pol[i].first;
			if (x2 < pol[i].first) x2 = pol[i].first;
			if (pol[i].second < y1) y1 = pol[i].second;
			if (y2 < pol[i].second) y2 = pol[i].second;
		}
	}
...
void RTree::SearchPointsInRect(const int a_min[2], const int a_max[2], vector<pair<int, int>>& result)
{
	...
	result.clear();
	SearchPointsInRectRec(m_root, &rect, result);
}


void RTree::SearchPointsInRectRec(Node* a_node, Rect* a_rect, vector<pair<int, int>>& result)
{
	if (a_node->IsInternalNode())
	{
		for (int index = 0; index < a_node->m_count; ++index)
		{
			...
			if (Overlap(a_rect, &branch->m_rect))
			{
				SearchPointsInRectRec(branch->m_child, a_rect, result);
			}
		}
	}
	else 
	{
		for (int index = 0; index < a_node->m_count; ++index)
		{
			...
			if (Overlap(a_rect, &branch->m_rect))
			{
				// Verificar si el punto esta dentro del rectangulo de busqueda
				for (const auto& point : branch->m_data)
				{
					if (point.first >= a_rect->m_min[0] && point.first <= a_rect->m_max[0] &&
						point.second >= a_rect->m_min[1] && point.second <= a_rect->m_max[1])
					{
...
void RTree::SearchRectanglesAtPoint(const int point[2], vector<vector<pair<int, int>>>& result)
{
	result.clear();
	SearchRectanglesAtPointRec(m_root, point, result);
}
...
void RTree::SearchRectanglesAtPointRec(Node* a_node, const int point[2], vector<vector<pair<int, int>>>& result)
{
	if (a_node->IsInternalNode())
	{
		...
		{
			Branch* branch = &a_node->m_branch[index];
			if (ContainsPoint(&branch->m_rect, point))
			{
				SearchRectanglesAtPointRec(branch->m_child, point, result);
			}
		}
	}
	else
...
bool RTree::ContainsPoint(const Rect* rect, const int point[2]) const
{
	return (point[0] >= rect->m_min[0] && 
			point[0] <= rect->m_max[0] &&
			point[1] >= rect->m_min[1] && 
...
{
	json j;
	Rect rect = NodeCover(m_root);
	j["rect"]["min"] = { rect.m_min[0], rect.m_min[1] };
	j["rect"]["max"] = { rect.m_max[0], rect.m_max[1] };
	ExportNodeToJson(m_root, j["root"]);
	// Guardar JSON
...
{
	jnode["level"] = node->m_level;
	jnode["count"] = node->m_count;
	jnode["branches"] = json::array();
	...
	for (int i = 0; i < node->m_count; ++i)
	{
		...
		jbranch["rect"]["min"] = { branch->m_rect.m_min[0], branch->m_rect.m_min[1] };
		jbranch["rect"]["max"] = { branch->m_rect.m_max[0], branch->m_rect.m_max[1] };

		if (node->IsInternalNode())
		{
			// Si es un nodo interno, exportar recursivamente
			jbranch["child"] = json::object();
			ExportNodeToJson(branch->m_child, jbranch["child"]);
		}
		else
		{
			// Si es un nodo hoja, agregar los puntos
			jbranch["data"] = json::array();
			for (const auto& point : branch->m_data)
			{
				jbranch["data"].push_back({ point.first, point.second });
			}
		}

		jnode["branches"].push_back(jbranch);
	}
}